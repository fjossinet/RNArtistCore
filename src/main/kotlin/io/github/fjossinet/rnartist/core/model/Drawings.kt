...
val radiusConst: Double = 7.0 //the radius for the residue circles
val spaceBetweenResidues: Double = 5.0 //the space between two consecutive residue circles in an helix
...
enum class LayoutProperty {
    radius, out_ids, rollBack, branch
}

fun helixDrawingLength(h: Helix) =
    (h.length - 1).toDouble() * radiusConst * 2.0 + (h.length - 1).toDouble() * spaceBetweenResidues

fun helixDrawingWidth() =
    radiusConst * deltaHelixWidth //from the center of a residue to the center of the paired residue
...
fun getStringBoundsRectangle2D(g: Graphics2D, s: String, font: Font): Dimension2D {
    g.font = font
    val fm = g.fontMetrics
    val lm = font.getLineMetrics(s, g.fontRenderContext)
    val r = fm.getStringBounds(s, g)
...
abstract class DrawingElement(
    ...
    var drawingConfiguration: DrawingConfiguration =
        DrawingConfiguration((this as? ResidueLetterDrawing)?.let { defaultResidueLetterConfiguration }
    ...
    open fun select(
        selector: (DrawingElement) -> Boolean,
        selection: MutableList<DrawingElement> = mutableListOf()
    ): List<DrawingElement> {
        if (selector(this))
            selection.add(this)
...
class SecondaryStructureDrawing(
    ...
    targetedJunctionBehaviors: List<Pair<Location, (JunctionDrawing, Int, ConnectorId) -> ConnectorId>>? = null,
    layout: Layout? = null
) {

    var overlappingScore = 0
    var bestLayoutScore = 0f
        get() = this.allJunctions.count { it.bestLayout == true }
            .toFloat() / this.allJunctions.count { it.junctionType != JunctionType.ApicalLoop }
            .toFloat()//number of junctions that have found their bestLayout (with the longest helix at north)
    val phosphoBonds =
    ...
    val branches = mutableListOf<BranchDrawing>() //the first junctions in each branch
    ...
    init {
        ...
        var currentPos = 0
        lateinit var lastBranchConstructed: BranchDrawing
        ...
        do {
            ...
            if (currentPos == 0) {
                ...
                lastBranchConstructed = BranchDrawing(
                    h,
                    this,
                    circles,
                    lines,
                    ...
                    targetedJunctionBehaviors = targetedJunctionBehaviors,
                    layout = layout
                )
                ...
                val newBranchConstructed = BranchDrawing(
                    h,
                    this,
                    circles,
                    lines,
        ...
        for (r in this.residues) {
            OUTER@ for (h in this.allHelices) {
                for (interaction in h.secondaryInteractions)
                    if (interaction.location.contains(r.location.start)) {
            ...
            if (r.parent == null) {
                for (j in this.allJunctions) {
                    if (j.location.contains(r.location.start)) {
                        r.parent = j
                        break
                    }
                }
            ...
            if (r.parent == null) {
                for (ss in this.allSingleStrands) {
                    if (ss.location.contains(r.location.start)) {
                        r.parent = ss
                        break
                    }
                }
    ...
    /**
     * Despite the fact that each junction has been computed to find a non-overlapping layout, some junction failed.
     * This function searches for overlapped elements (helcies and junctions), tags them (sets the boolean attribute overlappedLayout) and produces an overlappingScore score
     */
    fun computeOverlapping(print:Boolean = false) {
        this.overlappingScore = 0
        val allJunctions = this.allJunctions
        allJunctions.forEach { it.overlappedLayout = false }
        val allHelices = this.allHelices
        FOR@ for (i in 0..allJunctions.size - 2) {
            for (j in i + 1..allJunctions.size - 1) {
                //junction-junction overlap
                if (circlesOverlap(allJunctions.get(i).circle, allJunctions.get(j).circle)) {
                    this.overlappingScore++
                    allJunctions.get(i).overlappedLayout = true
                    continue@FOR
                }
            }
            val junction1Occupancy = getJunctionOccupancy(allJunctions.get(i).circle)

            for (j in 0..allHelices.size - 1) {
                val h = allHelices.get(j)
                if (allJunctions.get(i).inHelix != h.helix && !allJunctions.get(i).outHelices.contains(h)) {
                    val helixOccupancy = getHelixOccupancy(h.line.p1, h.line.p2, h.length)

                    //junction-helix overlap (if the helix h tested in not linked to the junction tested)
                    if (helixOccupancy.any { circlesOverlap(junction1Occupancy, it) }) {
                        this.overlappingScore++
                        allJunctions.get(i).overlappedLayout = true
                        continue@FOR
                    }

                    //helix-helix overlap (if the helix h tested in not linked to the junction tested)
                    for (outHelix in allJunctions.get(i).outHelices) {
                        if (getHelixOccupancy(
                                outHelix.line.p1,
                                outHelix.line.p2,
                                outHelix.length
                            ).any { r -> helixOccupancy.any { circlesOverlap(r, it) } }
                        ) {
                            if (print)
                                println("${outHelix.location} - ${h.location}")
                            this.overlappingScore++
                            allJunctions.get(i).overlappedLayout = true
                            continue@FOR
                        }
        ...
        val lastJunctionOccupancy = getJunctionOccupancy(allJunctions.last().circle)
        for (j in 0..allHelices.size - 1) {
            val h = allHelices.get(j)
            if (allJunctions.last().inHelix != h.helix && !allJunctions.last().outHelices.contains(h)) {
                val helixOccupancy = getHelixOccupancy(h.line.p1, h.line.p2, h.length)

                //junction-helix overlap (if the helix h tested in not linked to the junction tested)
                if (helixOccupancy.any { circlesOverlap(lastJunctionOccupancy, it) }) {
                    this.overlappingScore++
                    allJunctions.last().overlappedLayout = true
                    break
                }

                //helix-helix overlap (if the helix h tested in not linked to the junction tested)
                for (outHelix in allJunctions.last().outHelices) {
                    if (getHelixOccupancy(
                            outHelix.line.p1,
                            outHelix.line.p2,
                            outHelix.length
    ...
    fun getResiduesFromAbsPositions(vararg positions: Int): List<ResidueDrawing> {
        ...
        for (r: ResidueDrawing in residues) {
            if (r.location.start in positions) {
                _residues.add(r)
            }
        }
        return _residues
    ...
    fun computeResidues(junction: JunctionDrawing) {

        for (helix in junction.helicesFromBranch()) {
            this.computeResidues(helix)
        }

        for (j in junction.junctionsFromBranch()) {
            for (b in j.junction.location.blocks) {
                var angle = angleFrom(
                    j.center,
    ...
    private fun computeResidues(helix: HelixDrawing) {
        ...
        val step = helixDrawingLength(helix.helix) / (helix.helix.length - 1).toDouble()
    ...
    fun getLayoutEl(withoutThoseJunctions: List<JunctionDrawing>? = null): LayoutEl {
        val el = LayoutEl()
        this.allJunctions.sortedBy { it.junction.location.start }.forEach { junctionDrawing ->
            if (junctionDrawing.junctionType != JunctionType.ApicalLoop && (withoutThoseJunctions?.none { it == junctionDrawing }
                    ?: true)) {
                with(el.addJunction()) {
                    this.setOutIds(junctionDrawing.currentLayout.map { it.name }.joinToString(separator = " "))
                    this.setRadius(Math.round(junctionDrawing.initialRadius * junctionDrawing.radiusRatio * 100.0) / 100.0)
                    this.addLocation().setLocation(junctionDrawing.junction.location)
                }
            }
        }
        this.branches.sortedBy { it.junction.location.start }.forEachIndexed() { index, branch ->
            with(el.addBranch()) {
                this.addLocation().setLocation(branch.location)
                this.setValue((branch.parent as HelixDrawing).line.p1.x)
            }
        }
        return el
    }

    fun junctionsToImprove(backward: Int = 0, excludeInnerLoops: Boolean = false): List<JunctionDrawing> {
        val junctionsToImprove = mutableSetOf<JunctionDrawing>()
        this.allJunctions.filter { it.junctionType != JunctionType.ApicalLoop && it.overlappedLayout }
            .forEach {
                var step = backward
                var junctionToImprove: JunctionDrawing? = it
                while (junctionToImprove != null) {
                    if (!excludeInnerLoops || junctionToImprove.junctionType != JunctionType.InnerLoop) {
                        step--
                        if (step < 0) {
                            junctionsToImprove.add(junctionToImprove)
                            break
                        }
                    }
                    junctionToImprove = junctionToImprove.previousJunction
                }
            }
        return junctionsToImprove.toList()
    }

    fun junctionsNotToImprove(): List<Location> {
        val junctionsNotToImprove = mutableListOf<Location>()
        this.allJunctions.filter { it.junctionType != JunctionType.ApicalLoop && !it.overlappedLayout }
            .forEach { junctionsNotToImprove.add(it.location) }
        return junctionsNotToImprove.toList()
    }
    ...
    fun asSVG(frame: Rectangle2D, selectionFrame: Rectangle2D? = null, outputFile: File? = null): String {
        ...
        val svgString = this.asSVG(frame, outputFile)
    ...
    fun asSVG(frame: Rectangle2D, outputFile: File? = null): String {
        val at = AffineTransform()
        at.translate(this.workingSession.viewX, this.workingSession.viewY)
        at.scale(this.workingSession.zoomLevel, this.workingSession.zoomLevel)
        ...
        svgBuffer.append(
            """
<defs>
    <linearGradient id="reactivities_scale"  x1="0%" x2="100%" y1="0%" y2="0%">
      <stop offset="0%" stop-color="lightyellow" />
      <stop offset="100%" stop-color="firebrick" />
    </linearGradient>
  </defs>

  <rect fill="url(#reactivities_scale)" x="${frame.width - 200}" y="${frame.height - 48}" width="150" height="25"/>
  <rect fill="none" x="${frame.width - 202}" y="${frame.height - 50}" width="154" height="29" stroke="black" stroke-width="1"/>
  <text text-anchor="middle" x="${frame.width - 200}" y="${frame.height - 5}">0</text>
  <text text-anchor="middle" x="${frame.width - 200 + 150}" y="${frame.height - 5}">1</text>
  <text text-anchor="start" x="${frame.width - 202}" y="${frame.height - 55}">Reactivity</text>
    ...
    fun asChimeraScript(outputFile: File) {
        ...
        val numberingSystem: List<Int> =
            this.secondaryStructure.rna.numbering_system?.values?.toList()
                ?: (1..this.secondaryStructure.rna.length).map { it }
        ...
        when (this.secondaryStructure.source) {
            is PDBSource -> {
                ...
                colors2residues.forEach { (colorCode, residues) ->
                    ...
                    residues.forEach {
                        command.append("${numberingSystem[it.numberingToDraw - 1]},")
...
abstract class ResidueDrawing(
    ...
    var updated = true //to force the recomputation of base-base interaction shapes
        set(value) {
            field = value
            if (value)
                ssDrawing.residuesUpdated.add(this.location.start)
        }
    //if we have a numbering system, we use it
    val numberingToDraw: Int
        get() = ssDrawing.secondaryStructure.rna.numbering_system?.keys?.elementAt(this.location.start-1) ?: this.location.start
    ...
    override fun draw(
        ...
        if (this.isFullDetails()) {
            ...
            if ((numberingToDraw % 5 == 0 || numberingToDraw == 1 || numberingToDraw == ssDrawing.length))
        ...
        letterNumbering.forEach {
            ...
            buffer.append(
                if (RnartistConfig.exportSVGWithBrowserCompatibility())
                    ...
                    };font-family:${ssDrawing.workingSession.fontName};font-size:${ssDrawing.workingSession.fontSize - 2};">${it.first}</text>"""
                else
    ...
    private fun drawNumbering(g: Graphics2D, at: AffineTransform) {
        ...
        if (e != null && p != null) {
            var transX = (e!!.bounds2D.width - numberDim.width).toFloat() / 2F
            ...
            if (cp >= 0) {
                ...
                while (i < n) {
                    ...
                    g.drawString(
                        "$numberingToDraw".substring(i, i + 1),
                        e!!.bounds2D.minX.toFloat() + transX,
                        e!!.bounds2D.minY.toFloat() + transY
                    )
                    letterNumbering.add(
                ...
                g.drawString(
                    "$numberingToDraw".substring(n - 1, n),
                    e!!.bounds2D.minX.toFloat() + transX,
                    e!!.bounds2D.minY.toFloat() + transY
                )
                letterNumbering.add(
                ...
                while (i < n) {
                    ...
                    g.drawString(
                        "$numberingToDraw".substring(n - 1 - i, n - i),
                        e!!.bounds2D.minX.toFloat() + transX,
                        e!!.bounds2D.minY.toFloat() + transY
                    )
                    letterNumbering.add(
    ...
    override fun applyTheme(
        theme: Theme,
        checkStopBefore: ((DrawingElement) -> Boolean)?,
        checkStopAfter: ((DrawingElement) -> Boolean)?
    ) {
        theme.configurations.forEach { configuration ->
            if (configuration.select(this) && configuration.propertyName.equals(ThemeProperty.fulldetails.toString()) && !configuration.propertyValue(
                    this
                ).equals(this.isFullDetails().toString())
            )
    ...
    override fun draw(
        ...
        if (this.isFullDetails())
            ...
            if (tertiaryInteractions.size % 2 == 0) {
                ...
                middlePoint2 =
                    centerFrom(middleInteraction1.pairedResidue.center, middleInteraction2.pairedResidue.center)
            } else {
...
class HelixDrawing(
    ...
    val maxBranchLength: Int
        get() = this.helix.branchLengthFromHere
    ...
    override val bounds: List<Point2D>
        get() {
            ...
            if (this.isFullDetails()) {
                ...
                var outsidePoints = pointsFrom(
                    bp.residue.center,
                    bp.pairedResidue.center,
                    -(bp.residue.getLineWidth() + radiusConst * 1.1)
                )
                var p1 = getPerpendicular(
                    outsidePoints.first,
                    outsidePoints.first,
                    outsidePoints.second,
                    -(bp.residue.getLineWidth() + radiusConst * 1.1)
                )
                var p2 = getPerpendicular(
                    outsidePoints.second,
                    outsidePoints.first,
                    outsidePoints.second,
                    -(bp.residue.getLineWidth() + radiusConst * 1.1)
    ...
    override fun draw(
        ...
        /*g.color = Color.BLACK
        getHelixOccupancy(this.line.p1, this.line.p2, this.length).forEach {
            g.draw(at.createTransformedShape(it))
        }*/
...
class SingleStrandDrawing(ssDrawing: SecondaryStructureDrawing, val ss: SingleStrand, start: Point2D, end: Point2D) :
    ...
    var previousBranch: BranchDrawing? = null
    var nextBranch: BranchDrawing? = null
    ...
    fun branchRelocated(branch: BranchDrawing, transX: Double) {
        if (branch == this.previousBranch) {
            this.line.setLine(this.line.p1.x + transX, this.line.p1.y, this.line.p2.x, this.line.p2.y)
            ssDrawing.residues[this.start - 1].center = Point2D.Double(
                ssDrawing.residues[this.start - 1].center.x + transX,
                ssDrawing.residues[this.start - 1].center.y
            )
            if (this.nextBranch == null) {
                this.line.setLine(this.line.p1.x, this.line.p1.y, this.line.p2.x + transX, this.line.p2.y)
                ssDrawing.residues[this.end - 1].center = Point2D.Double(
                    ssDrawing.residues[this.end - 1].center.x + transX,
                    ssDrawing.residues[this.end - 1].center.y
                )
            }
            this.computeResidues()
        } else if (branch == this.nextBranch) {
            this.line.setLine(this.line.p1.x, this.line.p1.y, this.line.p2.x + transX, this.line.p2.y)
            ssDrawing.residues[this.end - 1].center = Point2D.Double(
                ssDrawing.residues[this.end - 1].center.x + transX,
                ssDrawing.residues[this.end - 1].center.y
    ...
    /**
    Compute the coordinates for all the residues in a single-strand
     */
    fun computeResidues() {
        if (this.start == 1) {
            if (this.length != 1) {
                val step = distance(
                    ssDrawing.residues[0].center,
                    ssDrawing.residues[this.end].center
                ) / (this.ss.length).toDouble()
                for (i in this.start + 1..this.end) {
                    val (p1_1, _) = pointsFrom(
                        ssDrawing.residues[0].center,
                        ssDrawing.residues[this.end].center,
                        step * (i - this.start).toDouble()
                    )
                    ssDrawing.residues[i - 1].center = p1_1
                }
            }
        } else if (this.end == ssDrawing.secondaryStructure.length) {
            if (this.length != 1) {
                val step = distance(
                    ssDrawing.residues[this.start - 2].center,
                    ssDrawing.residues[ssDrawing.secondaryStructure.length - 1].center
                ) / (this.length).toDouble()
                for (i in this.start until this.end) {
                    val (p1_1, _) = pointsFrom(
                        ssDrawing.residues[this.start - 2].center,
                        ssDrawing.residues[ssDrawing.secondaryStructure.length - 1].center,
                        step * (i - (this.start - 1).toDouble())
                    )
                    ssDrawing.residues[i - 1].center = p1_1
                }
            }
...
/**
 * @param unbalancedCutoff a double value between 0.0 and 1.0. If for at least one block making this junction, the length is below the total length * unbalancedCutoff, the out helices will be located according to the length between them (percent)
 */
open class JunctionDrawing(
    parent: HelixDrawing,
    ssDrawing: SecondaryStructureDrawing,
    junctionsFromBranchSoFar: MutableList<JunctionDrawing>,
    helicesFromBranchSoFar: MutableList<HelixDrawing>,
    ...
    var overlappedLayout = false
    var bestLayout = false
    ...
    //return the rank (from 1 to #helices -1) of helix starting the longest branch (number of residues betwenn start and end of the helices)
    val longestHelixRank: Int
        get() {
            var longest =
                this.junction.helicesLinked.sortedByDescending { it.end - it.start + 1 }[1] //the inHelix is logically the longest, so we want the second one once the linked helices sorted
            //do we have several helices with the same branch size?
            val allLongest =
                this.junction.helicesLinked.filter { it.end - it.start + 1 == longest.end - longest.start + 1 }
            //we return the helix at the middle (if only one helix, then it is this one)
            longest = allLongest[allLongest.size / 2]
            return this.junction.helicesLinked.indexOf(longest)
        }

    val outIdForLongest: ConnectorId
        get() {
            return this.currentLayout.get(longestHelixRank - 1)
        }

    val isUnbalanced: Boolean
        get() = this.junction.location.blocks.any {
            it.length.toFloat() - 2.0 < (this.junction.location.blocks.sumOf { it.length - 2 }
                .toFloat() / this.junction.helicesLinked.size.toFloat()) * 0.1f
        }
    ...
    val maxBranchLength: Int
        get() = this.junction.branchLengthFromHere

    init {
        if (!helicesFromBranchSoFar.contains(parent)) //this is necessary to add the parent helix of the first junction in the branch. After that, each junction will add its outHelices (and then the inHelices of the next junctions)
            helicesFromBranchSoFar.add(parent)
        this.residues =
        ...
        val circumference = junctionCircumference(
            this.junction,
            if (junctionSizeDelta.second.any { it.contains(this.junction.locationWithoutSecondaries) }) junctionSizeDelta.first else 0f
        )
        this.radius = circumference / (2F * Math.PI)
        this.initialRadius = circumference / (2F * Math.PI)

        var preDefinedLayout: List<ConnectorId>? = null
                ...
                if (configuration.selector(this)) {
                    when (configuration.propertyName) {
                        LayoutProperty.out_ids.toString() -> {
                            ...
                            preDefinedLayout = connectors.toMutableList()
                        }
                    }
                }
            }
        ...
        var helixRank = 0
        var formerOutId = inId
        //println("\n\nJunction ${this.name} ${this.junction.location}")
        ...
        for (k in 1..this.junction.helicesLinked.size + 1) {
            ...
            if (helix == (this.parent as HelixDrawing).helix) {
                junctionsFromBranchSoFar.add(this)
                break
            }
            helixRank += 1
            var inPoint: Point2D

            //println("helixRank $helixRank")
            ...
            var outId = preDefinedLayout?.let {
                ...
                targetedJunctionBehaviors?.let { targetedJunctionBehavior ->
                    if (targetedJunctionBehavior.any { it.first.contains(junction.locationWithoutSecondaries) }) {
                        targetedJunctionBehavior.find { it.first.contains(junction.locationWithoutSecondaries) }!!
                            .second(
                                this,
                                helixRank,
                        ...
                        junctionsBehaviors[this.junctionType]?.let {
                            it(
                                this,
                                helixRank,
                                outIdForLongest(junction.locationWithoutSecondaries)
            ...
            if (outId != null) {
                ...
                if (preDefinedLayout == null) {
                    ...
                    from = getConnectorId((formerOutId.value + 1) % ConnectorId.entries.size)
                    val remainingHelicesAfterThisOne = this.junction.helicesLinked.size - 1 - helixRank
                    val d = Math.abs(inId.value - 1 - remainingHelicesAfterThisOne)
                    to = if (inId.value - 1 - remainingHelicesAfterThisOne < 0)
                        getConnectorId(ConnectorId.entries.size - d)
                    else
                        getConnectorId(inId.value - 1 - remainingHelicesAfterThisOne)

                    var orientationsToTest = mutableListOf(from)
                    while (orientationsToTest.last() != to) {
                        val nextC = nextConnectorId(orientationsToTest.last())
                        if (nextC == inId) {//we have reached the inId helix, we cannot go further
                            break
                        }
                        orientationsToTest.add(nextC)
                    }

                    var indexOfPreferedOutId = orientationsToTest.indexOf(outId)

                    if (indexOfPreferedOutId == -1) {//this means that the preferred outId is outside of the from->to interval
                        //we're searching to which end of the orientations to test the preferred outID was the closest
                        val diffBefore = Math.abs(outId.value - orientationsToTest.first().value)
                        val diffAfter = Math.abs(outId.value - orientationsToTest.last().value)
                        if (diffBefore < diffAfter) {
                            outId = orientationsToTest.first()
                            indexOfPreferedOutId = 0
                        } else {
                            outId = orientationsToTest.last()
                            indexOfPreferedOutId = orientationsToTest.size - 1
                        }
                    }
                    val beforeOrientations = orientationsToTest.subList(0, indexOfPreferedOutId)
                    val afterOrientations = orientationsToTest.subList(indexOfPreferedOutId, orientationsToTest.size)
                    orientationsToTest =
                        interleaveArrays(
                            afterOrientations,
                            beforeOrientations.reversed()
                        ).toMutableList()//we will test at first the preferred outId
                    ...
                    do {
                        ...
                        //now we want to predict if the next junction and its inHelix (which is the helix we're processing for this junction with the current rank and outId) will overlap any helix or junction already computed

                        val nextCircumference = junctionCircumference(
                            nextJunction!!,
                            if (junctionSizeDelta.second.any { it.contains(nextJunction.locationWithoutSecondaries) }) junctionSizeDelta.first else 0f
                        )
                        ...
                        val nextJunctionOccupancy = getJunctionOccupancy(nextCircle)
                        val nextHelixOccupancy = getHelixOccupancy(this.connectors[outId.value], inPoint, helix.length)

                        //testing the overlap between former junctions and next helix and junction for this rank
                        outerloop@ for (formerJunctionDrawing in junctionsFromBranchSoFar) {
                            if (formerJunctionDrawing != this) {
                                val formerJunctionOccupancy = getJunctionOccupancy(formerJunctionDrawing.circle)
                                if (circlesOverlap(formerJunctionOccupancy, nextJunctionOccupancy)) {
                                    fine = false
                                    break@outerloop
                                }
                                if (nextHelixOccupancy.any { circlesOverlap(formerJunctionOccupancy, it) }) {
                                    fine = false
                                    break@outerloop
                                }
                            }
                        ...
                        //testing the overlap between former helices and next helix and junction for this rank
                        if (fine) {
                            outerloop@ for (formerHelixDrawing in helicesFromBranchSoFar) {

                                val formerHelixOccupancy =
                                    getHelixOccupancy(
                                        formerHelixDrawing.line.p1,
                                        formerHelixDrawing.line.p2,
                                        formerHelixDrawing.length
                                    )

                                if (junction.helicesLinked.contains(formerHelixDrawing.helix)) {

                                    if (linkedHelicesIntersect(
                                            formerHelixDrawing,
                                            this.connectors[outId.value],
                                            inPoint
                                        )
                                    ) {
                                        //if it is a test between two linked helices, they overlap if the lines at the bottom intersects
                                        fine = false
                                        break@outerloop
                                    }

                                    if (formerHelixOccupancy.any { circlesOverlap(nextJunctionOccupancy, it) }) {
                                        fine = false
                                        break@outerloop
                                    }
                                } else {

                                    if (nextHelixOccupancy.any { r ->
                                            formerHelixOccupancy.any {
                                                circlesOverlap(
                                                    it,
                                                    r
                                                )
                ...
                junctionsFromBranchSoFar.add(this)
                helicesFromBranchSoFar.add(h)

                nextJunction?.let {
                    this.connectedJunctions[outId] = JunctionDrawing(
                        h,
                        ssDrawing,
                        junctionsFromBranchSoFar = junctionsFromBranchSoFar,
                        helicesFromBranchSoFar = helicesFromBranchSoFar,
                ...
                junctionsFromBranchSoFar.add(this) //this is an apical loop that is added to check if the next junctions could overlap it
            }
        }
    ...
    /**
     * To translate this junction and eveything linked to it, this function translates the connector corresponding to the inId and then call update to recompute everything
     */
    fun translate(transX: Double) {
        this.connectors[this.inId.value] =
            Point2D.Double(this.connectors[this.inId.value].x + transX, this.connectors[this.inId.value].y)
        this.update()
    }
        ...
        layout.configurations.forEach { configuration ->
            if (configuration.selector(this)) {
                when (configuration.propertyName) {
                    ...
                    LayoutProperty.branch.toString() -> {
                        (this as BranchDrawing).relocate(configuration.propertyValue!!.toDouble())
                    }
    ...
    override fun draw(
        ...
        /*g.color = Color.BLACK
        g.draw(at.createTransformedShape(getJunctionOccupancy(this.circle)))*/
...
class BranchDrawing(
    ...
    junctionSizeDelta,
    outIdForLongest,
    targetedJunctionBehaviors,
    layout
) {
    /**
     * This function moves the line from the helix starting this branch to point p
     */
    fun relocate(x: Double) {
        //we just need to translate the parent helix and this junction/branch to let the drawing engind recompute the layout for the full branch
        val transX = x - (this.parent as HelixDrawing).line.p1.x
        with(this.parent as HelixDrawing) {
            this.line.setLine(
                this.line.p1.x + transX,
                this.line.p1.y,
                this.line.p2.x + transX,
                this.line.p2.y
            )
        }
        //we translate ths junction/branch (this will update the layout)
        translate(transX)
        //we recompute the residues coords for the entire branch
        ssDrawing.computeResidues(this)
        //we translate single-strand after each branch (or the first single-strand, since no branch before)
        ssDrawing.allSingleStrands.forEach {
            it.branchRelocated(this, transX)
        }
    }
}
...
abstract class BaseBaseInteractionDrawing(
    ...
    override val bounds: List<Point2D>
        get() {
            val l = mutableListOf<Point2D>()
            val outsidePoints = pointsFrom(
                this.residue.center,
                this.pairedResidue.center,
                -(this.residue.getLineWidth() + radiusConst * 1.1)
            )
            val p1 = getPerpendicular(
                outsidePoints.first,
                outsidePoints.first,
    ...
    override fun draw(
        ...
        if (residue.updated) {//the paired residue is de facto updated too
            ...
            if ((parent as HelixDrawing).distanceBetweenPairedResidues > 2 * shift) {
                ...
                if (this.isCanonical && false) { //here the false has to be replaced with a param to set if we want the historical symbol for AU and GC
                    if (isDoublePaired) {
...
class TertiaryInteractionDrawing(
    ...
    override fun draw(
        ...
        if (this.residue.numberingToDraw in ssDrawing.residuesUpdated || this.pairedResidue.numberingToDraw in ssDrawing.residuesUpdated) {
        ...
        if (this.isFullDetails() && (this.ssDrawing.workingSession.locationDrawn.contains(this.residue.numberingToDraw) && this.ssDrawing.workingSession.locationDrawn.contains(
                this.pairedResidue.numberingToDraw
            ))
        ) {
...
open class PhosphodiesterBondDrawing(
    ...
    override fun draw(
        ...
        if (parent is JunctionDrawing) {
            g.draw(
                at.createTransformedShape(
                    getArcBetween(this.residue, this.nextResidue)
                )
            )
        } else {
            g.draw(
                at.createTransformedShape(
                    getStraightLineBetween(this.residue, this.nextResidue)
                )
            )
        }
        g.stroke = previousStroke
    ...
    open fun asSVG(at: AffineTransform, frame: Rectangle2D): String {
        if (parent is JunctionDrawing) {
            val dist = distance((this.parent as JunctionDrawing).center, this.residue.center)
            val circle = Ellipse2D.Double(
                (this.parent as JunctionDrawing).center.x - dist,
                (this.parent as JunctionDrawing).center.y - dist,
                dist * 2.0,
                dist * 2.0
            )

            val arc = getArcBetween(this.residue, this.nextResidue)
            val p1 = Point2D.Double()
            val p2 = Point2D.Double()
            val _c = at.createTransformedShape(circle)
            at.transform(this.residue.center, p1)
            at.transform(this.nextResidue.center, p2)
            return """<path d="M ${p1.x} ${p1.y} A ${_c.bounds2D.width / 2.0} ${_c.bounds2D.width / 2.0} ${arc.angleStart} 0 1 ${p2.x} ${p2.y}" stroke="${
                getHTMLColorString(
                    this.getColor()
                )
            }" stroke-width="${
            ...
            val straightLine = getStraightLineBetween(this.residue, this.nextResidue)
            val p1 = Point2D.Double()
            val p2 = Point2D.Double()
            at.transform(straightLine.p1, p1)
            at.transform(straightLine.p2, p2)
...
class InHelixClosingPhosphodiesterBondDrawing(
    ...
    override fun draw(
        ...
        g.color = this.getColor(selectedDrawings)
        val center1 =
            if (this.posInhelix == this.residue.location.start && !this.residue.parent!!.parent!!.isFullDetails()) (this.residue.parent?.parent as HelixDrawing).line.p2 else this.residue.center
        val center2 =
            if (this.posInhelix == this.nextResidue.location.start && !this.nextResidue.parent!!.parent!!.isFullDetails()) (this.nextResidue.parent?.parent as HelixDrawing).line.p2 else this.nextResidue.center

        if (this.residue.parent is SecondaryInteractionDrawing && !this.residue.parent!!.parent!!.isFullDetails() || this.nextResidue.parent is SecondaryInteractionDrawing && !this.nextResidue.parent!!.parent!!.isFullDetails()) {
            g.draw(at.createTransformedShape(getStraightLineBetween(center1, center2)))
        } else {
            g.draw(
                at.createTransformedShape(
                    getArcBetween(center1, center2, this.parent as JunctionDrawing)
                )
            )
        }
        g.stroke = previousStroke
    ...
    override fun asSVG(at: AffineTransform, frame: Rectangle2D): String {
        ...
        if (this.residue.parent is SecondaryInteractionDrawing && !this.residue.parent!!.parent!!.isFullDetails() || this.nextResidue.parent is SecondaryInteractionDrawing && !this.nextResidue.parent!!.parent!!.isFullDetails()) {
            val straightLine = getStraightLineBetween(center1, center2)
            val p1 = Point2D.Double()
            val p2 = Point2D.Double()
            at.transform(straightLine.p1, p1)
            at.transform(straightLine.p2, p2)
            ...
            val dist = distance((this.parent as JunctionDrawing).center, center1)
            val circle = Ellipse2D.Double(
                (this.parent as JunctionDrawing).center.x - dist,
                (this.parent as JunctionDrawing).center.y - dist,
                dist * 2.0,
            ...
            val arc = getArcBetween(center1, center2, this.parent as JunctionDrawing)
            val p1 = Point2D.Double()
            val p2 = Point2D.Double()
            val _c = at.createTransformedShape(circle)
            at.transform(this.residue.center, p1)
...
class OutHelixClosingPhosphodiesterBondDrawing(
    ...
    override fun draw(
        ...
        val center1 =
            if (this.posInhelix == this.residue.location.start && !this.residue.parent!!.parent!!.isFullDetails()) (this.residue.parent?.parent as HelixDrawing).line.p1 else this.residue.center
        val center2 =
            if (this.posInhelix == this.nextResidue.location.start && !this.nextResidue.parent!!.parent!!.isFullDetails()) (this.nextResidue.parent?.parent as HelixDrawing).line.p1 else this.nextResidue.center
...
class SingleStrandLinkingBranchPhosphodiesterBondDrawing(
    ...
    override fun draw(
        ...
        val center1 =
            when {
                this.residue.numberingToDraw == this.posInHelix && !this.residue.parent!!.parent!!.isFullDetails() -> (this.residue.parent!!.parent as HelixDrawing).line.p1
                else -> this.residue.center
            }
        val center2 =
            when {
                this.nextResidue.numberingToDraw == this.posInHelix && !this.nextResidue.parent!!.parent!!.isFullDetails() -> (this.nextResidue.parent!!.parent as HelixDrawing).line.p1
...
class HelicesDirectLinkPhosphodiesterBondDrawing(
    ...
    override fun draw(
        ...
        val previousStroke = g.stroke
        g.stroke = BasicStroke(
            this.ssDrawing.zoomLevel.toFloat() * this.getLineWidth().toFloat(), BasicStroke.CAP_SQUARE,
            BasicStroke.JOIN_ROUND
        )
        g.color = this.getColor(selectedDrawings)

        val center1 =
            if (!this.residue.parent!!.parent!!.isFullDetails()) (if (this.posForP2 == this.residue.location.start) (this.residue.parent?.parent as HelixDrawing).line.p2 else (this.residue.parent?.parent as HelixDrawing).line.p1) else this.residue.center
        val center2 =
            if (!this.nextResidue.parent!!.parent!!.isFullDetails()) (if (this.posForP2 == this.nextResidue.location.start) (this.nextResidue.parent?.parent as HelixDrawing).line.p2 else (this.nextResidue.parent?.parent as HelixDrawing).line.p1) else this.nextResidue.center

        with(getArcBetween(center1, center2, this.parent as JunctionDrawing)) {
            g.draw(at.createTransformedShape(this))
        }
...
/**
 * Return the next connector id after connector c given as parameter and according to the size of the piece of junction linking both connectors (given as a percent of the junction full size)
 */
fun connectorIdFrom(c: ConnectorId, percentOfJunctionSize: Float, outHelices: Int): ConnectorId {
    val nextC =
        ConnectorId.entries.first { it.value == (c.value + (ConnectorId.entries.size.toFloat() * percentOfJunctionSize).toInt()) % ConnectorId.entries.size }
    return if (nextC.value == c.value) //if the percent is too low, this could return c
        nextConnectorId(c)
    else
        nextC
}
    ...
    Pair(JunctionType.ThreeWay,
        { junctionDrawing: JunctionDrawing, helixRank: Int, outIdForLongest: ConnectorId ->
            ...
            if (dist_InId_OutIdLongest < helicesBeforelongest) {
                //if we don't have enough space for to place the helices before the longest, we need to push the longest after the north which was what we wanted
                outIdForLongest =
                ...
                outIdForLongest =
                    if (outIdForLongest.value - spaceNeeded < 0) getConnectorId(16 - spaceNeeded) else getConnectorId(
                        outIdForLongest.value - spaceNeeded
                    )
            }
...
fun distance(drawing1: SecondaryStructureDrawing, drawing2: SecondaryStructureDrawing): Float {
    val junctions1 =
        drawing1.allJunctions.sortedBy { it.location.start }.filter { it.junctionType != JunctionType.ApicalLoop }
    val junctions2 =
        drawing2.allJunctions.sortedBy { it.location.start }.filter { it.junctionType != JunctionType.ApicalLoop }
    var total = 0
    for (i in 0 until junctions1.size) {
        total += Math.abs(junctions1.get(i).outIdForLongest.value - junctions2.get(i).outIdForLongest.value)
    }
    return total.toFloat() / junctions1.size.toFloat()
}
...
/**
Return two new points far from p1 and b2 by dist. If dist is a negative value, the two new points are outside.
 **/
...
/**
 * Computes perpendicular points to a line (p1,p2) and intesecting it at p0
 */
...
fun getStraightLineBetween(residue: ResidueDrawing, nextResidue: ResidueDrawing): Line2D {
    val center1 = residue.center
    val center2 = nextResidue.center
    val (p1, p2) = pointsFrom(
        center1,
        center2,
        radiusConst + deltaPhosphoShift
    )
    return getStraightLineBetween(
        if (!residue.willBeDrawn()) center1 else p1,
        if (!nextResidue.willBeDrawn()) center2 else p2
    )
}

fun getStraightLineBetween(p1: Point2D, p2: Point2D): Line2D {
...
fun getArcBetween(residue: ResidueDrawing, nextResidue: ResidueDrawing): Arc2D {
    val junctionDrawing = residue.parent as? JunctionDrawing ?: nextResidue.parent as JunctionDrawing
    val center1 = residue.center
    val center2 = nextResidue.center
    return getArcBetween(center1, center2, junctionDrawing)
}

fun getArcBetween(p1: Point2D, p2: Point2D, j: JunctionDrawing): Arc2D {
    val dist = distance(j.center, p1)
    val circle = Ellipse2D.Double(j.center.x - dist, j.center.y - dist, dist * 2.0, dist * 2.0)
    val ref = Point2D.Double(j.center.x + dist, j.center.y)
    val cp = crossProduct(j.center, p1, p2)
    val cp2 = crossProduct(j.center, p1, ref)
    val a1 = angleFrom(j.center, ref, p1)
    var a2 = angleFrom(j.center, p1, p2)
    return if (cp2 <= 0) {
        if (cp > 0)
            Arc2D.Double(circle.bounds2D, 360 - a1 - a2, a2, Arc2D.OPEN)
        else {
            a2 = 360 - a2
            Arc2D.Double(circle.bounds2D, 360 - a1 - a2, a2, Arc2D.OPEN)
        }
        ...
        if (cp > 0)
            Arc2D.Double(circle.bounds2D, a1, -a2, Arc2D.OPEN)
        else {
            a2 = 360 - a2
            Arc2D.Double(circle.bounds2D, a1, -a2, Arc2D.OPEN)
        }
...
fun linkedHelicesIntersect(formerHelix: HelixDrawing, lineStart: Point2D, lineEnd: Point2D): Boolean {
    val p1 = getPerpendicular(
        lineStart,
        lineEnd,
        lineStart,
...
/**
 * Two circles overlap if the distance between their center is less to the sum of their radius
 */
fun circlesOverlap(c1: Ellipse2D.Double, c2: Ellipse2D.Double): Boolean {
    val center1 = Point2D.Double(c1.centerX, c1.centerY)
    val center2 = Point2D.Double(c2.centerX, c2.centerY)
    return distance(center1, center2) < c1.width / 2.0 + c2.width / 2.0
}

/**
 * An helix occupancy is made with consecutive circles along its middle line.
 */
fun getHelixOccupancy(lineStart: Point2D, lineEnd: Point2D, helixLength: Int): List<Ellipse2D.Double> {
    val centers = mutableListOf<Point2D>()
    val radius = helixDrawingWidth() / 2.0 + radiusConst
    if (helixLength == 2) { // we need a single circle at the center of the helix
        val center = pointsFrom(lineStart, lineEnd, distance(lineStart, lineEnd)/2.0).first
        return listOf(Ellipse2D.Double(center.x - radius, center.y - radius, radius * 2.0, radius * 2.0))
    } else {
        centers.add(lineStart)
        for (i in 1 until helixLength) {
            if (i == 1)
                centers.add(pointsFrom(centers[0], lineEnd, 2.0 * radiusConst + spaceBetweenResidues).first)
            else
                centers.add(
                    pointsFrom(
                        centers[i - 2],
                        centers[i - 1],
                        -2.0 * radiusConst - spaceBetweenResidues
                    ).second
                )
        }
        val circles =
            centers.map { Ellipse2D.Double(it.x - radius, it.y - radius, radius * 2.0, radius * 2.0) }.toMutableList()
        circles.removeFirst() //otherwise the occupancy expand too much at this end
        circles.removeLast() //otherwise the occupancy expand too much at this end
        return circles
    }

}

fun getJunctionOccupancy(circle: Ellipse2D) =
    Ellipse2D.Double(
        circle.x - (radiusConst * 4.0) / 2.0,
        circle.y - (radiusConst * 4.0) / 2.0,
        (circle.width + radiusConst * 4.0),
        (circle.height + radiusConst * 4.0)
    )
